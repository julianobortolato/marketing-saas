...
must_haves:
  truths:
    ...
    - "Tenant A authenticated user cannot read Tenant B's academia_config (RLS RESTRICTIVE proven)"
  artifacts:
...
Trigger reconciliation (REQUIRED, not optional): the Plan 01 `handle_new_user` trigger fires on every auth.users INSERT and creates a NEW tenant + owner usuario. For an invited teammate that extra tenant is wrong. After `inviteUserByEmail`, use the admin client to reconcile the invited user's `public.usuarios` row so its `tenant_id` = the CALLER's tenant_id (from `getCurrentUsuario()`, never from the request body) and its `role` = the requested role (idempotent upsert keyed on `usuarios.id`). This is a real working reconciliation delivered in Phase 1 — document it in the SUMMARY as a known Phase 1 simplification (a future migration may make the trigger invite-aware via `raw_user_meta_data`). Do not label it a placeholder/v1/v2 in code.
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
1. `npm run dev`. As an owner (tenant A), POST to `/api/invite` (via curl or browser fetch) inviting `gestor@academia.com.br` as `manager` and `leitor@academia.com.br` as `viewer`. Confirm 200 and that each invited user's usuarios row has tenant_id = tenant A (SQL editor).
...
6. Confirm the automated grep gate below passes (service key never reachable from UI/components).
  ...
  <resume-signal>Type "approved" once: invited manager logged in and reached the dashboard, viewer saw the form read-only with no save button AND the DB rejected a viewer write, and tenant B could not read tenant A's academia_config; otherwise describe the failure.</resume-signal>
</task>
